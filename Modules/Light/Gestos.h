...
int8_t signoX, signoY; //para el golpe horizontal (toma valores 1 o -1)
...
void gestos() {
  ...
  switch (gestoEnCurso)
  {
    case NINGUNO: //si no hay un gesto en curso, reconocer uno nuevo
      // Los gestos están ordenados de menor a mayor
      ...
      // Para detectar el golpe horizontal se deben seguir varios pasos
      // Primero ver si alguno de los ejes (X o Y) se satura
      if (AcX <= SATN or AcX >= SATP or AcY <= SATN or AcY >= SATP) {
        gestoEnCurso = GOLPE_H; // Inicia el reconocimento del golpe vertical
        // También se guardan los signos de los ejes,
        // el próximo pico deberá saturar en el signo contrario
        signoX = (AcX >= 0) ? 1 : -1;
        signoY = (AcY >= 0) ? 1 : -1;
        tFin = millis() + T_ESP_PICOS; //asignar tiempo límite
        gestoPicoNum = 2; //se va a detectar el segundo pico
    ...
    case GOLPE_H: //se está reconiciendo un golpe horizontal
      if (gestoPicoNum == 2) { //se va a reconocer el segundo pico
        if ((signoX > 0 and AcX <= SATN) //primero positivo y ahora negativo
        or  (signoX < 0 and AcX >= SATP) //primero negativo y ahora positivo
        or  (signoY > 0 and AcY <= SATN)
        or  (signoY < 0 and AcY >= SATP)) {
          tFin = millis() + T_ESP_PICOS; //nuevo tiempo límite
          gestoPicoNum = 3; //sigue el tercer pico
        }
      }
      else if (gestoPicoNum == 3) { //se va a reconocer el tercer pico
        if ((signoX > 0 and AcX >= SATP) //primero positivo y ahora otra vez positivo
        or  (signoX < 0 and AcX <= SATN) //primero negartivo y ahora otra vez negativo
        or  (signoY > 0 and AcY <= SATN)
        or  (signoY < 0 and AcY >= SATP)){
          gestoReco = GOLPE_H; //se ha reconicido un golpe horizontal
          gestoPicoNum = 1; //reinicar número
          gestoEnCurso = NINGUNO;
          tGesto = millis() + T_ESP_GESTOS; //esperar antes de reconocer otro gesto
        }
